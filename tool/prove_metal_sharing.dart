...
Future<void> main(List<String> args) async {
  ...
  final scenarios = const <_ScenarioSpec>[
    _ScenarioSpec(
      key: 'metal_full',
      packageName: 'objc_metal_full_tmp',
      mode: _MetalMode.full,
    ),
    _ScenarioSpec(
      key: 'metal_full_no_comments',
      packageName: 'objc_metal_full_no_comments_tmp',
      mode: _MetalMode.fullNoComments,
    ),
    _ScenarioSpec(
      key: 'metal_full_no_categories',
      packageName: 'objc_metal_full_no_categories_tmp',
      mode: _MetalMode.fullNoCategories,
    ),
    _ScenarioSpec(
      key: 'metal_full_no_protocols',
      packageName: 'objc_metal_full_no_protocols_tmp',
      mode: _MetalMode.fullNoProtocols,
    ),
    _ScenarioSpec(
      key: 'metal_prefix_only',
      packageName: 'objc_metal_prefix_only_tmp',
      mode: _MetalMode.prefixOnly,
    ),
    _ScenarioSpec(
      key: 'metal_prefix_only_no_comments',
      packageName: 'objc_metal_prefix_only_no_comments_tmp',
      mode: _MetalMode.prefixOnlyNoComments,
    ),
  ];
  ...
  try {
    ...
    for (final spec in scenarios) {
      ...
      final scenario = _Scenario(
        spec: spec,
        packageDir: packageDir,
        bindingFile: File('${packageDir.path}/lib/src/metal_bindings.dart'),
      );
      results.add(
...
BenchmarkOptions _parseArgs(List<String> args) {
  ...
  for (final arg in args) {
    switch (arg) {
      ...
      case '--help':
        stdout.writeln(
          'Usage: dart run tool/prove_metal_sharing.dart [--keep-workdir]',
        );
        exit(0);
      default:
  ...
  return BenchmarkOptions(
    repoRoot: repoRoot,
    workDir: Directory('${Directory.systemTemp.path}/dart_objc_metal_variants'),
...
Future<void> _generateMetalPackage({
  required Directory repoRoot,
  required Directory packageDir,
  required String packageName,
  required String sdkPath,
  required _MetalMode mode,
}) async {
  ...
  final objectiveC = switch (mode) {
    _MetalMode.full || _MetalMode.fullNoComments => const ObjectiveC(
    ...
    _MetalMode.fullNoCategories => const ObjectiveC(
      interfaces: Interfaces(
        include: Declarations.includeAll,
        includeTransitive: true,
      ),
    ...
    _MetalMode.fullNoProtocols => const ObjectiveC(
      interfaces: Interfaces(
        include: Declarations.includeAll,
        includeTransitive: true,
      ),
    ...
    _MetalMode.prefixOnly || _MetalMode.prefixOnlyNoComments => ObjectiveC(
  ...
  final commentType = switch (mode) {
    _MetalMode.fullNoComments ||
    _MetalMode.prefixOnlyNoComments => const CommentType.none(),
  ...
  final generator = FfiGenerator(
    ...
    output: Output(
      dartFile: Uri.file('${packageDir.path}/lib/src/metal_bindings.dart'),
      objectiveCFile: Uri.file('${packageDir.path}/native/metal_bindings.m'),
      commentType: commentType,
      preamble: '// Metal benchmark package.',
...
''');
  ...
  ).writeAsStringSync("export 'src/metal_bindings.dart';\n");
}
...
void main() {
  final manager = metal.MTLCaptureManager.sharedCaptureManager();
  stdout.writeln(manager.ref.pointer.address);
}
''');
  await _runChecked(
    workingDirectory: appDir.path,
