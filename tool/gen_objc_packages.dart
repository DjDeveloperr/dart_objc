...
void _generateBindings(_FrameworkSpec spec, Directory pkgDir, String sdkPath) {
  ...
  final sharedImports = spec.key == 'appkit'
      ? _loadFoundationImports(pkgDir.parent)
      : const _SharedImports.empty();

  final generator = FfiGenerator(
    headers: Headers(
      entryPoints: [header],
      include: (header) => _isFrameworkHeader(spec, header),
      compilerOptions: _compilerOptionsFor(spec, sdkPath),
      ignoreSourceErrors: true,
    ),
    objectiveC: _objectiveCConfig(spec),
    // TODO(dart-lang/native#2596): Move this to declaration-level imports
    // when the ffigen configuration API supports them.
    // ignore: deprecated_member_use
    importedTypesByUsr: sharedImports.typesByUsr,
    // TODO(dart-lang/native#2597): Infer this from imported types when the
    ...
    output: Output(
      ...
      symbolFile: spec.key == 'foundation'
          ? SymbolFile(
              Uri.parse('package:objc_foundation/objc_foundation.dart'),
              Uri.file('${pkgDir.path}/lib/src/foundation_symbols.yaml'),
            )
          : null,
      preamble:
...
bool _isFrameworkHeader(_FrameworkSpec spec, Uri header) {
  final frameworkDirectory = '${spec.framework}.framework';
  final segments = header.pathSegments;
  final frameworkIndex = segments.indexOf(frameworkDirectory);
  return frameworkIndex >= 0 &&
      segments.skip(frameworkIndex + 1).contains('Headers');
}

_SharedImports _loadFoundationImports(Directory packagesDir) {
  final symbolsFile = File(
    '${packagesDir.path}/objc-foundation/lib/src/foundation_symbols.yaml',
  );
  if (!symbolsFile.existsSync()) {
    throw StateError(
      'Foundation symbols are missing. Run '
      '`dart run tool/gen_objc_packages.dart foundation` before AppKit.',
    );
  }

  final document = loadYaml(symbolsFile.readAsStringSync()) as YamlMap;
  ...
  for (final fileEntry in files.nodes.entries) {
    final importPath = (fileEntry.key as YamlScalar).value as String;
    final library = LibraryImport('foundation${importIndex++}', importPath);
    libraries.add(library);
    final file = (fileEntry.value as YamlMap).value;
    final symbols = file['symbols'] as YamlMap;
    for (final symbolEntry in symbols.nodes.entries) {
      final usr = (symbolEntry.key as YamlScalar).value as String;
      if (!_shareFoundationSymbol(usr)) continue;
...
bool _shareFoundationSymbol(String usr) =>
    usr.startsWith('c:objc(cs)') ||
    usr.startsWith('c:objc(cy)') ||
    usr.startsWith('c:objc(pl)') ||
    usr.startsWith('c:@E@');

List<String> _compilerOptionsFor(_FrameworkSpec spec, String sdkPath) {
  final clangResourceDir = _firstLineOfStdout('xcrun', [
    'clang',
    '-print-resource-dir',
  ]);
  return [
    '-isystem',
    '$clangResourceDir/include',
    '-isysroot',
    sdkPath,
...
dependencies:
$extraDeps$frameworkDeps  ffi: ^2.2.0
  objective_c:
...
String _libraryExports(_FrameworkSpec spec) {
  ...
  if (spec.key == 'appkit') {
    buffer.writeln("export 'package:objc_foundation/objc_foundation.dart';");
  }
  buffer.writeln("export 'src/${spec.generatedBase}_bindings.dart';");
  if (spec.key == 'appkit') {
