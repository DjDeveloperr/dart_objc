...
const _assetName = 'objc_uikit_flutter_views.dylib';
...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (codeConfig.targetOS != OS.iOS) return;
    ...
    final flags = <String>[
      ...
      '-mios-version-min=${codeConfig.iOS.targetVersion}',
    ];
...
String _sdkPath(CodeConfig config) {
  final sdk = config.iOS.targetSdk == IOSSdk.iPhoneOS
      ? 'iphoneos'
      : 'iphonesimulator';
...
String _targetTriple(CodeConfig config) {
  ...
  return switch ((architecture, sdk)) {
    (Architecture.arm64, IOSSdk.iPhoneOS) => 'arm64-apple-ios',
    (Architecture.arm64, IOSSdk.iPhoneSimulator) => 'arm64-apple-ios-simulator',
    (Architecture.x64, IOSSdk.iPhoneSimulator) => 'x86_64-apple-ios-simulator',
    _ => throw UnsupportedError('Unsupported iOS target: $architecture / $sdk'),
  };
}
